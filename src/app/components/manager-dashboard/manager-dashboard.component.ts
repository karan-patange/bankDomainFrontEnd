 ...
 currentView: string = 'pending';
   ...
   constructor(private http: HttpClient, private sanitizer: DomSanitizer,) {}
       ...
       rejectRequest(): void {
         ...
         this.http.post(`http://localhost:9292/api/bank/manager/reject/${this.selectedRequestId}`, requestBody,{ responseType: 'text' }).subscribe({
           next: () => {
       ...
       viewDocuments(id: number) {
         ...
         this.http
           .get(`http://localhost:9292/api/bank/manager/request/${id}/documents`, {
             responseType: 'json',
           })
           .subscribe({
             next: (response: any) => {
